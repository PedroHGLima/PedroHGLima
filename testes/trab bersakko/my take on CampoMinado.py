    ...
    while not lost and not win:
        ...
        for i in range(len(listabombas)):
            if posicaoEscolhidaX==listabombas[i][0] and posicaoEscolhidaY==listabombas[i][1]:
                ...
                while True:
                    resposta = input ('Deseja ver onde estavam as bombas? (s/n): ')
                    if resposta == 's':
...
def main():
    ...
    while True:
        ...
        #verifica que opcao foi escolhida no input 'opcao'    
            ...
            if opcao == 2:
                print('\nObservacao: jogos inacabados nao tem sua pontuacao salva.')
                ...
                if len(listanomefacil)>0:
                    print ('\nPontuacao campo minado facil:')
                    while pos<len(listanomefacil):
                        print('Jogador: '+listanomefacil[pos]+'\nPontos: '+listapontuacaofacil[pos])
                        pos=pos+1
                else:
                    print ('\nNenhuma pontuacao do modo facil encontrada')

                #verifica se ha algum elemento na lista, para, entao, mostrar a pontuacao         
                if len(listanomemedio)>0:
                    pos=0        
                    print ('\nPontuacao campo minado medio:')
                    while pos<len(listanomemedio):
                        print('Jogador: '+listanomemedio[pos]+'\nPontos: '+listapontuacaomedio[pos])
                        pos=pos+1
                else:
                    print ('\nNenhuma pontuacao do modo medio encontrada')
